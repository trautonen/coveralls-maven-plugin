...
* Supports [Cobertura](http://mojo.codehaus.org/cobertura-maven-plugin/),
  ...
  [Saga](http://timurstrekalov.github.io/saga/) and [Scoverage](http://scoverage.org/) coverage tools
...
Scoverage is supported via its ability to generate Cobertura compatible reports.

Setup scoverage in the build section of the project pom.xml:
...
In your maven-coveralls-plugin section of the POM, ensure that you set the coberturaReports configuration directory setting to point to where the cobertura reports are generated by scoverage: 
...
Execute Maven to create Scoverage report and submit Coveralls data:

```
after_success:
- mvn clean test coverage:report coveralls:report
```
